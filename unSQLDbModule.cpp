...
__fastcall TSqlDBModule::TSqlDBModule(TComponent* Owner) : TDataModule(Owner) {
	...
	pathUDL = ExtractFilePath(Application->ExeName) + "WorkBD.udl";
...
int TSqlDBModule::GetIntFieldSQL(AnsiString _tableName, AnsiString _fieldName, AnsiString _where, int _default, int &err) {
	...
	try {
		...
		if (_where == NULL) {
			ADOQueryDB->SQL->Text = "select " + Trim(_fieldName) + " from " + _tableName;
		}
		else {
			ADOQueryDB->SQL->Text = "select " + Trim(_fieldName) + " from " + _tableName + " where " + _where;
		}

		ADOQueryDB->Open();
		strCheck = ADOQueryDB->FieldByName(_fieldName)->AsString;
		if (strCheck == "") {
			result = _default;
			err = -2;
		}
...
bool TSqlDBModule::GetBoolFieldSQL(AnsiString _tableName, AnsiString _fieldName, AnsiString _where, bool _default, int &err) {
	...
	try {
		...
		ADOQueryDB->Open();
		result = ADOQueryDB->FieldByName(Trim(_fieldName))->AsBoolean;
		ADOQueryDB->Close();
		err = 0;
		// return result;
	}
...
double TSqlDBModule::GetFloatFieldSQL(AnsiString _tableName, AnsiString _fieldName, AnsiString _where, double _default, int &err) {
	...
	try {
		...
		ADOQueryDB->Open();
		strTmp=ADOQueryDB->FieldByName(_fieldName)->AsString;
		if (strTmp=="") {
		 result = _default;
		}else{
...
AnsiString TSqlDBModule::GetStrFromFunctionSql(AnsiString _strSql, int &_err) {
	// int _err = -1;
	AnsiString result = "";
	try {
		if (!ADOConnectionDB->Connected) {
		...
		ADOQueryDB->Open();
		_err = ADOQueryDB->RecordCount;
		if (ADOQueryDB->FieldByName("F1")->AsString == "") {
			_err=-1;
			result = "-1";
		}
		else {
...
double TSqlDBModule::GetDoubleFromFunctionSql(AnsiString _strSql, int &_err) {
	// int _err = -1;
	double result = 0;
	try {
		if (!ADOConnectionDB->Connected) {
...
int TSqlDBModule::FillComboBox(AnsiString _tableName, AnsiString _fieldName, AnsiString _where, TComboBox* _comboBox) {
	int result = 0;
	int rec_id = -1;
	AnsiString str = "";
	if (!ADOConnectionDB->Connected) {
	...
	if (_where == NULL) {
		ADOQueryDB->SQL->Text = "select rec_id," + _fieldName + " from " + _tableName + " order by rec_id";
	}
	else {
		ADOQueryDB->SQL->Text = "select rec_id," + _fieldName + " from " + _tableName + " where " + _where + " order by rec_id";
	}
...
void __fastcall TSqlDBModule::ADOQueryDBAfterClose(TDataSet *DataSet) { // восстановим клиентский курсор на всякий случай
...
int TSqlDBModule::UpdIntSql(AnsiString _tableName, AnsiString _fieldName, int _fieldValue, AnsiString _where) {
	...
	try {
		...
		if (_where == NULL) {
			strSql = "UPDATE " + _tableName + " SET " + _fieldName + " = " + IntToStr(_fieldValue);
		}
		else {
			strSql = "UPDATE " + _tableName + " SET " + _fieldName + " = " + IntToStr(_fieldValue) + " WHERE " + _where;
		}
		ADOQueryDB->SQL->Text = strSql;
		ADOQueryDB->ExecSQL();
		err = ADOQueryDB->RowsAffected;
...
int TSqlDBModule::UpdBoolSql(AnsiString _tableName, AnsiString _fieldName, bool _fieldValue, AnsiString _where) {
	int err = -1;
	AnsiString strSql = "";
	try {
		if (!ADOConnectionDB->Connected) {
		...
		if (_where == NULL) {
			strSql = "UPDATE " + _tableName + " SET " + _fieldName + " = " + IntToStr((int)_fieldValue);
		}
		else {
			strSql = "UPDATE " + _tableName + " SET " + _fieldName + " = " + IntToStr((int)_fieldValue) + " WHERE " + _where;
		}
		ADOQueryDB->SQL->Text = strSql;
		ADOQueryDB->ExecSQL();
		err = ADOQueryDB->RowsAffected;
