...
// ---------------------------------------------------------------------------
void __fastcall TfmShowListTubes::FormClose(TObject *Sender,
	TCloseAction &Action) {
...
void __fastcall TfmShowListTubes::menuShowZonesClick(TObject *Sender) {
	...
	if (f == 0 || t == 0) {
		MessageDlg("Данных по трубе нет!", mtWarning,
...
void __fastcall TfmShowListTubes::FormCreate(TObject *Sender) {
 ...
 ", T5.steelGradeName as 'Марка стали', T6.typeSizeName as 'Типоразмер'" \
 ", T7.workShiftName as 'Смена'" \
 ",T2.operatorName as 'Оператор'" \
 ",(CASE WHEN NOT EXISTS(SELECT * FROM resultThick where " \
 "numFusion=T1.numFusion and numTube=T1.numTube) THEN 'Нет данных' ELSE" \
 " case T1.resultT when 0 then 'Годно' when -1 then 'Нет данных' " \
 "else 'ДЕФЕКТ' end end) as 'К. Толщины' " \
 ",(CASE WHEN NOT EXISTS(SELECT * FROM resultCross where " \
 "numFusion=T1.numFusion and numTube=T1.numTube) THEN 'Нет данных' ELSE" \
 " case T1.resultC when 0 then 'Годно' when -1 then 'Нет данных' else" \
 " 'ДЕФЕКТ' end end) as 'Поперечный к.' " \
 ",(CASE WHEN NOT EXISTS(SELECT * FROM resultLong where " \
 "numFusion=T1.numFusion and numTube=T1.numTube) THEN 'Нет данных' ELSE" \
 " case T1.resultL when 0 then 'Годно' when -1 then 'Нет данных' else" \
 " 'ДЕФЕКТ' end end) as 'Продольный к.' " \
 ",T1.lengthTube as 'Длина трубы',T1.speedTube as 'Скорость'" \
 ",(case T1.isSOP when 0 then 'Нет' when 1 then 'Да' else" \
 " 'Неизвестно' end) as 'СОП'" \
 " ,T1.thresholdC1,T1.thresholdC2,T1.thresholdL1" \
 ",T1.thresholdL2,thresholdTUp,thresholdTDown,thresholdTNominal" \
 ",T1.pathFileNameThick as 'Файл МНК1'" \
 ",T1.pathFileNameCross as 'Файл МНК2'" \
 ",T1.pathFileNameLong as 'Файл МНК3'" \
 ",T1.indOperatorName,T1.indNormDocName,T1.indSopName," \
 "T1.indSteelGradeName,T1.indWorkShiftName,T1.indTypeSize,T1.dtmCreate" \
 " ,YEAR(T1.dtmCreate) as Y1, MONTH(T1.dtmCreate) as M1" \
 ", DATEPART( dayofyear , T1.dtmCreate ) as DY1" \
 ",YEAR(T1.dtmCreate) as Y2, MONTH(T1.dtmCreate) as M2," \
 " DATEPART( dayofyear , T1.dtmCreate ) as DY2" \
 "	,T1.resultC   " \
 "	,T1.numFusion " \
 "	,T1.resultL   " \
 "	,T1.resultT   " \
 ...
 " where T1.numTube>0 and T1.isEmpty=0 " \
 "and T1.numFusion=:pnumFusion  and resultT<>-100" \
 " order by  T1.numFusion, T1.numTube ";

		lbeFuzion->Text = IntToStr(TGlSettings::currFusion);
		queryShowListTubes->Close();
		queryShowListTubes->SQL->Text = strSqlGlobal;
		queryShowListTubes->Parameters->ParamByName("pnumFusion")->Value =
			StrToInt(lbeFuzion->Text);
		...
		strSql =
			" SELECT colNum,colWidth FROM gridTubesColWidth order by colNum ";
		SqlDBModule->queryQuick->SQL->Text = strSql;
		SqlDBModule->queryQuick->Open();
		SqlDBModule->queryQuick->First();
		for (int i = 0; i < DBGridListTubes->Columns->Count; i++) {
			DBGridListTubes->Columns->Items[i]->Width =
				SqlDBModule->queryQuick->FieldByName("colWidth")->AsInteger;
		...
		// --------
		SqlDBModule->FillComboBox("tubesTypeSize", "typeSizeName", "isActive=1",
			cbxFTubesTypeSize);
		cbxFTubesTypeSize->AddItem("Не выбрано", NULL);
		cbxFTubesTypeSize->ItemIndex = (cbxFTubesTypeSize->Items->Count - 1);
		// cbxSteelGrades
		SqlDBModule->FillComboBox("steelGrades", "steelGradeName", "isActive=1",
			cbxSteelGrades);
		...
		bbtFiltered->Caption = "Фильтр Найдено:" +
			IntToStr(queryShowListTubes->RecordCount) + " записей";
		err = 0;
	}
	catch (Exception *ex) {
		err = -1;
...
void __fastcall TfmShowListTubes::bbtFilteredClick(TObject *Sender) {
	...
	try {
		...
		if (lbeFuzion->Text == "ВСЕ" || lbeFuzion->Text == "*") {
			queryShowListTubes->Close();
			strSqlGlobal = StringReplace(strSqlGlobal,
				"T1.numFusion=:pnumFusion", "T1.numFusion is Not NULL",
			...
			strSqlGlobal = StringReplace(strSqlGlobal,
				"T1.numFusion is Not NULL", "T1.numFusion=:pnumFusion",
				ReplaceFlags << rfReplaceAll << rfIgnoreCase);
		...
		if (cbxBrackAll->Checked) {
			...
			strSqlGlobal = StringReplace(strSqlGlobal, " and resultT<>-100 ",
				" and (resultT>0 or resultC>0 or resultL>0) ",
				ReplaceFlags << rfReplaceAll << rfIgnoreCase);
		}
		else {
			strSqlGlobal = StringReplace(strSqlGlobal,
		...
		if (cbxOperators->ItemIndex == (cbxOperators->Items->Count - 1)) {
			...
			strFilter += " and [indOperatorName] = " +
				IntToStr((int)cbxOperators->Items->Objects
				[cbxOperators->ItemIndex]);
		}
		if (cbxNormDocs->ItemIndex == (cbxNormDocs->Items->Count - 1)) {
			//
		}
		else {
			strFilter += " and [indNormDocName] = " +
				IntToStr((int)cbxNormDocs->Items->Objects
				[cbxNormDocs->ItemIndex]);
		}
		if (cbxSops->ItemIndex == (cbxSops->Items->Count - 1)) {
			//
		}
		else {
			strFilter += " and [indSopName] = " +
				IntToStr((int)cbxSops->Items->Objects[cbxSops->ItemIndex]);
		}
		if (cbxSteelGrades->ItemIndex == (cbxSteelGrades->Items->Count - 1)) {
			//
		}
		else {
			strFilter += " and [indSteelGradeName] = " +
				IntToStr((int)cbxSteelGrades->Items->Objects
				[cbxSteelGrades->ItemIndex]);
		}
		if (cbxFTubesTypeSize->ItemIndex ==
			(cbxFTubesTypeSize->Items->Count - 1)) {
			//
		}
		else {
			strFilter += " and [indTypeSize] = " +
				IntToStr((int)cbxFTubesTypeSize->Items->Objects
				[cbxFTubesTypeSize->ItemIndex]);
		}
...
void __fastcall TfmShowListTubes::bbtResetFiltrClick(TObject *Sender) {
	...
	try {
		...
		if (err != 0) {
			...
			queryShowListTubes->Open();
			bbtFiltered->Caption =
				"Фильтр Найдено:" + IntToStr(queryShowListTubes->RecordCount) +
				" записей";
			// выдернем ширины столбцов
			SqlDBModule->queryQuick->Close();
			strSql =
...
int TfmShowListTubes::OutToRtf() {
	...
	strOut = "\tБУРАН 9955 Сводный отчет неразрушающего контроля от: " +
	...
	strOut += "\t\t" + Trim(queryShowListTubes->FieldByName
		("Нормативный документ")->AsString);
	strTmp = Trim(queryShowListTubes->FieldByName("Нормативный документ")
		->AsString);
	...
	// strOut += "\t" + Trim(queryShowListTubes->FieldByName("Марка стали")->AsString);
	// }
	strOut += "\t\t" + Trim(queryShowListTubes->FieldByName("Марка стали")
		->AsString);
	...
	while (!queryShowListTubes->Eof) {
		strOut = "\t" + queryShowListTubes->FieldByName("Номер трубы")
			->AsString;
		strOut += "\t\t" + queryShowListTubes->FieldByName
			("Дата и время контроля")->AsString;
		strOut += "\t\t" + queryShowListTubes->FieldByName("Длина трубы")
			->AsString;
		strTmp = queryShowListTubes->FieldByName("Длина трубы")->AsString;
		if (strTmp.Length() > 5) {
			strOut += "\t\t" + queryShowListTubes->FieldByName("К. толщины")
				->AsString;
		}
		else {
			strOut += "\t\t\t" + queryShowListTubes->FieldByName("К. толщины")
		...
		if (strTmp.Length() > 5) {
			strOut += "\t\t\t" + queryShowListTubes->FieldByName
				("Поперечный к.")->AsString;
		}
		else {
			strOut += "\t\t" + queryShowListTubes->FieldByName("Поперечный к.")
				->AsString;
		}
		...
		if (strTmp.Length() > 5) {
			strOut += "\t\t\t" + queryShowListTubes->FieldByName
				("Продольный к.")->AsString;
		}
		else {
			strOut += "\t\t" + queryShowListTubes->FieldByName("Продольный к.")
				->AsString;
		}
...
int TfmShowListTubes::OutToRtfStream() {
	...
	try {
		...
		fmReport->streamRtf->WriteString
			("{\\f0\\fmodern\\fprq1\\fcharset204 Courier New;} \n");
		fmReport->streamRtf->WriteString
			("{\\f1\\fmodern\\fprq1\\fcharset204 Courier New;}} \n"); // шрифты
		fmReport->streamRtf->WriteString
		...
		fmReport->streamRtf->WriteString
			("\\pard\\widctlpar\\li-567\\sa200\\sl276\\slmult1\\qc \n");
		fmReport->streamRtf->WriteString
			("БУРАН 9955 Сводный отчет неразрушающего контроля ");
		fmReport->streamRtf->WriteString("\\par");
		fmReport->streamRtf->WriteString
			("\\pard\\widctlpar\\fi1275\\li-567\\sa200\\sl276\\slmult1\\fs24 \n"
			);
		...
		fmReport->streamRtf->WriteString
			(IntToStr(queryShowListTubes->RecordCount));
		fmReport->streamRtf->WriteString("\\par\n");
		// таблица заголовки
		fmReport->streamRtf->WriteString
			("\\trowd\\trgaph108\\trleft176\\trbrdrl\\brdrs\\brdrw10 \\trbrdrt\\brdrs\\brdrw10 \n"
			);
		fmReport->streamRtf->WriteString
		...
		fmReport->streamRtf->WriteString("\\pard\\intbl\\widctlpar");
		fmReport->streamRtf->WriteString
			(" " + queryShowListTubes->FieldByName("Номер плавки")->AsString);
		fmReport->streamRtf->WriteString("\\cell");
		fmReport->streamRtf->WriteString
			(" " + Trim(queryShowListTubes->FieldByName("Типоразмер")
			->AsString));
		fmReport->streamRtf->WriteString("\\cell");
		fmReport->streamRtf->WriteString
			(" " + Trim(queryShowListTubes->FieldByName("Нормативный документ")
			->AsString));
		fmReport->streamRtf->WriteString("\\cell");
		fmReport->streamRtf->WriteString
			(" " + Trim(queryShowListTubes->FieldByName("Марка стали")
			->AsString));
		fmReport->streamRtf->WriteString("\\cell\\row\n"); // конец строки 2
		// ----------------
		fmReport->streamRtf->WriteString
			("\\pard\\widctlpar\\li-567\\sa200\\sl276\\slmult1\\par ");
		// конец таблицы
		fmReport->streamRtf->WriteString("\\par\n");
		// // ----------------------------
		// таблица 2 заголовки
		...
		fmReport->streamRtf->WriteString("\\pard\\intbl\\widctlpar");
		fmReport->streamRtf->WriteString
			(" " + Trim(queryShowListTubes->FieldByName("Настр. СОП")
			->AsString));
		fmReport->streamRtf->WriteString("\\cell");
		fmReport->streamRtf->WriteString
			(" " + Trim(queryShowListTubes->FieldByName("Оператор")->AsString));
		...
		while (!queryShowListTubes->Eof) {
			if (queryShowListTubes->RecNo % countRows1 == 0 && numPage == 1) {
				...
				if ((queryShowListTubes->RecNo - countRows1)
					% countRows2 == 0 && numPage > 1) {
					numPage++;
					fmReport->streamRtf->WriteString("\\par\n");
					fmReport->streamRtf->WriteString
						("\\pard\\widctlpar\\li-567\\sa200\\sl276\\slmult1\\par "
			...
			fmReport->streamRtf->WriteString
				...
				queryShowListTubes->FieldByName("Дата и время контроля")
				->AsDateTime));
			fmReport->streamRtf->WriteString("\\cell");
			fmReport->streamRtf->WriteString
				(" " + queryShowListTubes->FieldByName("Номер трубы")
				->AsString);
			fmReport->streamRtf->WriteString("\\cell");
			fmReport->streamRtf->WriteString
				(queryShowListTubes->FieldByName("К. толщины")->AsString);
			fmReport->streamRtf->WriteString("\\cell");
			fmReport->streamRtf->WriteString
				(queryShowListTubes->FieldByName("Поперечный к.")->AsString);
			fmReport->streamRtf->WriteString("\\cell");
			fmReport->streamRtf->WriteString
				(queryShowListTubes->FieldByName("Продольный к.")->AsString);
			fmReport->streamRtf->WriteString("\\cell\\row\n"); ;
			// конец строки 2
			// ----------------
			queryShowListTubes->Next();
		...
		fmReport->streamRtf->SaveToFile
			("e:\\PRG\\BURAN-9955PaWork\\Help\\stream.rtf");
...
// ---------------------------------------------------------------------------
void __fastcall TfmShowListTubes::FormKeyDown(TObject *Sender, WORD &Key,
	TShiftState Shift) {
	TShiftState shift;
	TMouseButton Button;
	if (Key == VK_ESCAPE) {
	...
	if (Key == VK_RETURN && queryShowListTubes->RecNo <
		queryShowListTubes->RecordCount && queryShowListTubes->RecNo > 0) {
		int t = queryShowListTubes->FieldByName("Номер трубы")->AsInteger;
		int f = queryShowListTubes->FieldByName("Номер плавки")->AsInteger;
		if (f == 0 || t == 0) {
			MessageDlg("Данных по трубе нет!", mtWarning,
...
void __fastcall TfmShowListTubes::FormResize(TObject *Sender) {
	...
	countRows1 = SqlDBModule->GetIntFieldSQL("currentSettings",
		"ParamValueFloat",
		"isActual = 1 and UPPER(ParamName) = UPPER('countRows1')", 0, err);
	countRows2 = SqlDBModule->GetIntFieldSQL("currentSettings",
		"ParamValueFloat",
...
void __fastcall TfmShowListTubes::FormCloseQuery(TObject *Sender,
	bool &CanClose) {
	int e = DBGridListTubes->Columns->Count;
	for (int i = 0; i < e; i++) {
		SqlDBModule->UpdIntSql("gridTubesColWidth", "colWidth",
			DBGridListTubes->Columns->Items[i]->Width,
			"colNum=" + IntToStr(i + 1));
	}
	queryShowTube->Close();
...
void __fastcall TfmShowListTubes::miDeleteTubeClick(TObject *Sender) {
	int numTube = queryShowListTubes->FieldByName("Номер трубы")->AsInteger;
    int numFusion = queryShowListTubes->FieldByName("Номер плавки")->AsInteger;
	if (MessageDlg("Запись по трубе с номером " + IntToStr(numTube) +
		" будет удалена", mtWarning, TMsgDlgButtons() << mbOK << mbCancel,
		0) == mrOk) {
